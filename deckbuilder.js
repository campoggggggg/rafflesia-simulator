...
function renderDeckBuilderScreen() {
  ...
  if (!("commanderId" in currentDeck)) {
    currentDeck.commanderId = null;
  }

  if (!("territoryCards" in currentDeck)) {
    currentDeck.territoryCards = [];
  }
...
function renderCardPreview() {
  ...
  const canSetCommander = isCommanderEligible;
  ...
  previewBox.innerHTML = `
    ...
    <div class="card-preview-actions">
      <button id="setCommanderBtn" class="secondary-btn" ${canSetCommander ? "" : "disabled"}>
...
function canAddCardToDeck(deck, card) {
  ...
  if (deck.commanderId && card.id === deck.commanderId) {
    return false;
  }
